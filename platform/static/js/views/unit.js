...
function actionsHtml(){
  ...
  if(step.kind === "learn"){
    return {info:["Теория", "Прочитал — идём дальше"], acts:nextBtn};
  }
  if(step.kind === "quiz"){
    const answered = C.quizFb[step.main.id] || quizDone(step.main.id);
    return answered
      ? {info:["Ответ засчитан", "Можно двигаться дальше"], acts:nextBtn}
      : {info:["Вопрос на понимание", "Выбери вариант — цифры 1–4 тоже работают"],
         acts:`<button class="btn btn--quiet" data-act="next">Пропустить</button>`};
  }
  ...
  return {
    ...
    acts:`${hintBtn}
      <button class="btn btn--ghost btn--sm" data-act="skip">Пропустить</button>
...
export function renderUnit(root, nav, unitIdx, stepIdx){
  ...
  C = {unitIdx, unit, steps, nav, cur:0, root:null, hints:{}, quizFb:{},
       busy:false, last:{}, sandboxSql:""};

  const schemaOpen = lsGet("schemaOpen") !== "false";
  ...
  delegate(node, "click", "[data-act]", async (e, btn) => {
    switch(btn.dataset.act){
      ...
      case "to-editor":
        ...
        setEditor(C.unit.blocks[+btn.dataset.ex].sql);
        $("#editorTa", node).focus();
        return;
